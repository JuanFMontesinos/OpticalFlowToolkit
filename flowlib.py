...
def read_flow(filename):
    ...
    if 202021.25 != magic:
        ...
        try:
            w = np.fromfile(f, np.int32, count=1)[0]     # For Python3.x
            h = np.fromfile(f, np.int32, count=1)[0]     # For Python2.x
        except:
...
def write_flow_png(flow,filename):
    ...
    write optical flow in 16bit 2-channel png file (grayscale + alpha)
    :param flow: optical flow map
    :param filename: optical flow file path to be saved
    :return: None
    """
    if not (flow.dtype == 'float32' or flow.dtype == 'float64'):
        raise ValueError('Float data required')
    if len(flow.shape) != 3:
        raise Exception('Input has more than 2 dimensions, wrong flow format')
    if flow.shape[2] != 2:
        raise Exception('Unexpected amount of channels: {0}. Expecting two channels u,v').format(flow.shape[2])
